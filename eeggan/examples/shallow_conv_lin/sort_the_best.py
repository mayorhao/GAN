...
def sortTheBest(train,fold_idx):
    ...
    batch_size=100
    mmds=[]
    data_path="./analysis/pick-8-layer-ss3/fold-{}/{}/synthesis.mat".format(fold_idx,stage)
    save_path = "./analysis/5-fold-8-layer-sorted/fold-{}-batch-size-100/{}/".format(fold_idx, stage)
    saved_dic=scio.loadmat(data_path)
    fake=saved_dic["x"]
    ...
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    scio.savemat(os.path.join(save_path,"ground_truth.mat"),{
        "x":sorted_fake,
        "mmd":mmds
    })
    print("done")
